...
void _parse_ports(ps_args_t *ps_args, char *ports) {
    ...
    int single_flag = 1; // if there's only one port
	...
	if (idx_1 == 0 && single_flag) {
		// port number is zero
		printf("Number 0 is a not valid port. \n");
		usage(stdout);
		exit(-1);
	}
	else if (idx_1 > 65536 && single_flag) {
		// too large 
		printf("Port number is too large. \n");
		usage(stdout);
		exit(-1);
